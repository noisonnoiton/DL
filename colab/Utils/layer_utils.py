...
def conv_forward(x, w, b, conv_param):
    ...
    - conv_param: A dictionary with the following keys:
      ...
      - 'padding': "valid" or "same". "valid" means no padding.
        "same" means padding the input so that the output has the shape as (N, ceil(H / SH), ceil(W / SW), F)
        If the padding on both sides are off by one, the bottom and right get the additional padding.
...
def Test_conv_forward():
    ...
    conv_param = {'stride': np.array([1,3,2,1]), 'padding': 'valid'}
    out, _ = conv_forward(x, w, b, conv_param)
    correct_out = np.array([[[[ 0.04733114, -0.094658  ]],
                             [[-0.29040982,  0.67798229]]],
                            [[[-0.51557047,  1.19307582]],
                             [[-0.85331144,  1.96571612]]]])
