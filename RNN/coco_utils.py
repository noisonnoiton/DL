...
def load_coco_data(base_dir='coco_captioning',
                                     max_train=None,
                                     pca_features=True):
    data = {}
...
def load_test_data(base_dir='coco_captioning',
                                     max_train=None,
                                     pca_features=True):
    data = {}
    caption_file = os.path.join(base_dir, 'test2014_captions.h5')
    with h5py.File(caption_file, 'r') as f:
        for k, v in f.items():
            data[k] = np.asarray(v)

    if pca_features:
        train_feat_file = os.path.join(base_dir, 'test2014_vgg16_fc7_pca.h5')
    else:
        train_feat_file = os.path.join(base_dir, 'test2014_vgg16_fc7.h5')
    with h5py.File(train_feat_file, 'r') as f:
        data['features'] = np.asarray(f['features'])
    ...
    train_url_file = os.path.join(base_dir, 'test2014_urls.txt')
    with open(train_url_file, 'r') as f:
        train_urls = np.asarray([line.strip() for line in f])
    data['urls'] = train_urls
